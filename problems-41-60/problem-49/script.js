...
function largestPathValue(word, edgeList){
  ...
  const incomingEdgeListCopy = new Map(); // For after toopological sort
  ...
  const neighbours = adjList.get(start);
  neighbours.add(end);
...
// find longest path in a directed acylic graph from topological ordering
...
// @param {Map<int, Set<int>>} adjList
// @param {Map<int, Set<int>>} incomingEdgeList
// @return {int[]}
...
for(let i = 0; i < nodes.length; i++){
	const node = nodes[i];
  // checking for no incoming edges from this node
  
  if(incomingEdgeList.get(node).size === 0) nodesWithNoIncomingEdges.push(node);
...
while(nodesWithNoIncomingEdges.length !== 0){
  ...
  const neighbours = Array.from(adjList.get(node));
  
  for(let i = 0; i < neighbours.length; i++){
	const neigbour = neigbours[i];
  const incomingEdgesFromNeighbour = incomingEdgeList.get(neighbour);
  incomingEdgesFromNeighbour.delete(node);
  
  if(incomingEdgesFromNeighbour.size === 0) nodesWithNoIncomingEdges.push(neighbour);
