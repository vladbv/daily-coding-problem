...
// Returning a sorted string ordering of courses such that we can finish all courses
...
function courseOrder(courses) {
    ...
    // dequeue course from queue and add to the ordering, then deleting that course from any of the courses that may depend on it.
    ...
    // checking if there are any dependencies left for all the courses. If there are, it is a cyclic graph, so there is no such ordering.
    for(let i = 0; i < coursesKeys.length; i++){
        const courseName = coursesKeys[i];
        if(courses.get(courseName).size !== 0) return [];
    }

    // no more dependencies are left, which means the ordering is correct
    return ordering;
}
